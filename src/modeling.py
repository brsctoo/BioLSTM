...
def slide_window(sample, window_size=120) -> list[list[int]]:
...
def save_XY_to_file(output_path, X_list, y_list):
    ...
    X = np.empty((total_samples, 120, 4), dtype=np.float16)
...
def extract_windows_numpy(seq_onehot, indices, window_size=120):
    """
