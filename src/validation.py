...
def smooth_predict(predict_raw, window_size=20):
    ...
    for i in range(len(predict_raw)):
        ...
        # If the majority of the surrounding neighbors are exons (1), force it to be an exon.
...
def validate_model(model_path, data_test, rf=None, threshold=0.70):
    ...
    for sample in data_test:
        ...
        if isinstance(predictions, list):
            predictions = predictions[-1] # Take the final output
        ...
        # 3. Masking: Ignore positions with -1 (out of bounds / unknown)
        mask = Y >= 0
    ...
    # 5. Aggregate Alignment Results by decile (0-10%, 10-20%, etc.)
    ...
    for metric in final_metrics:
        ...
        if identity_value <= 0.1: q0 += 1
        elif identity_value <= 0.2: q1 += 1
        elif identity_value <= 0.3: q2 += 1
        elif identity_value <= 0.4: q3 += 1
        elif identity_value <= 0.5: q4 += 1
        elif identity_value <= 0.6: q5 += 1
        elif identity_value <= 0.7: q6 += 1
        elif identity_value <= 0.8: q7 += 1
        elif identity_value <= 0.9: q8 += 1
        else: q9 += 1
   ...
   # 6. Keras standard ML metrics
    ...
    # Update states for all metrics using the true vs predicted labels
